...
type View = 'main' | 'email-login' | 'email-signup';

export default function LoginModal() {
  const { showLoginModal, setShowLoginModal, login, loginWithEmail, signUpWithEmail } = useAuth();
  ...
  const handleLogin = async (provider: 'discord' | 'google') => {
    ...
    setTimeout(() => setLoggingIn(null), 5000);
  };
        ...
        {view === 'main' && (
          <div className="flex flex-col gap-3 pt-4">
            ...
            <p className="text-center text-xs text-muted-foreground mt-2">
              ...
              <button className="text-primary hover:underline" onClick={() => setView('email-signup')}>
                Sign up
              </button>
            </p>
          </div>
        ...
        {view === 'email-signup' && (
          <div className="flex flex-col gap-3 pt-4">
            ...
            <p className="text-center text-xs text-muted-foreground">
              ...
              <button className="text-primary hover:underline" onClick={() => setView('email-login')}>
                Sign in
              </button>
            </p>
          </div>
